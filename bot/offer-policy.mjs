function valid(c,categoryId,recentIds){
  try{const product=new URL(c.permalink),image=new URL(c.imageUrl);return c.status==="active"&&(c.scopeCategoryId??c.categoryId)===categoryId&&!recentIds.has(c.itemId)&&typeof c.title==="string"&&c.title.trim()&&Number.isFinite(c.price)&&c.price>0&&product.protocol==="https:"&&/(^|\.)mercadolivre\.com\.br$/.test(product.hostname)&&/MLB-?\d+/i.test(product.pathname)&&image.protocol==="https:"&&/(^|\.)mlstatic\.com$/.test(image.hostname);}catch{return false;}
}
const discount=c=>Number.isFinite(c.originalPrice)&&c.originalPrice>c.price?(c.originalPrice-c.price)/c.originalPrice:0;
export function selectOffers(candidates,{categoryId,recentIds,limit}){const seen=new Set();return candidates.filter(c=>valid(c,categoryId,recentIds)&&!seen.has(c.itemId)&&seen.add(c.itemId)).sort((a,b)=>discount(b)-discount(a)||a.rank-b.rank||a.itemId.localeCompare(b.itemId)).slice(0,limit);}
